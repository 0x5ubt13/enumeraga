...
func printCloudUsage() {
	...
	fmt.Println("      --tenant ID          Azure Tenant ID (optional service principal auth; see note below)")
	fmt.Println("      --client-id ID       Azure Client/App ID (optional service principal auth; see note below)")
	fmt.Println("      --client-secret SEC  Azure Client Secret (optional service principal auth; see note below)")
	fmt.Println("                           Azure: omit all three to scan unattended as your signed-in user")
	fmt.Println("                           via 'az login' (ScoutSuite + Prowler). Supply all three to add a")
	fmt.Println("                           service principal — required for monkey365's M365/Entra inventory.")
